...
function pageMarkers(value: string): string[] {
  const output: string[] = [];
  const patterns = [
  ...
  for (const pattern of patterns) {
    for (const match of value.matchAll(pattern)) if (match[1]) output.push(String(Number(match[1])));
  }
...
function verifyPincite(
  ...
  const blockIndexes = quote.sourceBlocks
    .map((id) => resolution.loaded?.blocks.findIndex((block) => block.id === id) ?? -1)
    .filter((index) => index >= 0);
  const firstIndex = blockIndexes.length ? Math.min(...blockIndexes) : 0;
  const contextBlocks = resolution.loaded.blocks.slice(Math.max(0, firstIndex - 1), Math.max(...blockIndexes, firstIndex) + 1);
  const markers = pageMarkers(contextBlocks.map((block) => block.text).join(" "));
  if (markers.includes(String(Number(requestedPage)))) {
    return {
      status: "verified",
      explanation: `The matched quotation occurs in source context carrying page marker ${Number(requestedPage)}.`,
      sourceBlocks: quote.sourceBlocks,
  ...
  return {
    status: "pincite_unverifiable",
    explanation: markers.length
      ? `Nearby source markers (${markers.join(", ")}) do not conclusively establish pinpoint ${occurrence.pincite}.`
      : "The saved opinion does not preserve a usable official page marker near the matched material.",
    sourceBlocks: quote.sourceBlocks,
    observedPage: markers.length === 1 ? markers[0] : undefined,
  };
}
...
function calculateOverall(result: VerifiedCitationOccurrence, checks: Set<VerificationCheck>): VerifiedCitationOccurrence["overallStatus"] {
