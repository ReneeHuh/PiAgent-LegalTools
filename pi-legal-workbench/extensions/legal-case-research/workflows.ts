...
export function documentSourceCandidates(item: NormalizedCase, requested: ProviderId | "auto" = "auto"): ProviderSource[] {
  ...
  const order: ProviderId[] = ["scholar", "courtlistener"];
...
function rawResultProviderId(provider: DiscoveryProviderId, raw: ProviderRawResult): string | undefined {
  const direct = provider === "scholar" ? (raw as { caseId?: unknown }).caseId : (raw as { clusterId?: unknown }).clusterId;
...
async function locateAndClickRenderedOpinion(
  ...
  if (!source.providerId || !/^\d+$/.test(source.providerId)) {
    throw new Error(`${source.provider} result has no numeric opinion identifier.`);
  }
