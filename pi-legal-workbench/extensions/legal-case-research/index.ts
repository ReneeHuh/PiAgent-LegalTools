...
const LegalSearchSchema = Type.Object({
  ...
  year_from: Type.Optional(Type.Integer({
    minimum: 1600,
    maximum: 2100,
    description: "Earliest filing year to include on both providers.",
  })),
  year_to: Type.Optional(Type.Integer({
    minimum: 1600,
    maximum: 2100,
    description: "Latest filing year to include on both providers.",
  })),
...
const OpenBrowserSchema = Type.Object({
  provider: Type.Union([Type.Literal("scholar"), Type.Literal("courtlistener")]),
}, {
  additionalProperties: false,
...
export default function legalSearchExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description:
      "Open or focus the persistent visible Google Scholar or CourtListener browser at its known HTTPS homepage. Use it for manual inspection or human verification; it does not run or resume a search and accepts no URL.",
    promptSnippet: "Open the Scholar or CourtListener browser",
    promptGuidelines: [
    ...
    async execute(toolCallId, params: OpenBrowserParams, signal, onUpdate) {
      return runWithToolStatus({
        ...
        operation: async (progress) => {
          ...
          if (provider !== "scholar" && provider !== "courtlistener") throw new Error(`Unsupported provider: ${provider}.`);
