...
export const SEARCH_PROVIDERS = ["scholar", "courtlistener"] as const;
...
export function validateLegalSearchOptions(options: LegalSearchOptions): ValidatedLegalSearchRequest {
  ...
  if (!SEARCH_PROVIDERS.includes(options.provider)) {
    throw new Error('provider must be either "scholar" or "courtlistener".');
  }
  ...
  const endPage = pagesToSearch === -1
    ? options.provider === "scholar" ? MAX_SEARCH_PAGE : Number.POSITIVE_INFINITY
...
function publicResults(
  ...
  return pageResults.map(({ page, position, case: item }) => {
    ...
    return {
      ...
      matching_passage: item.snippet ?? null,
      passage_source: item.snippet ? "provider_snippet" : "unavailable",
      case_key: representative.canonicalKey,
      provider,
      provider_case_id: source?.providerId,
      opinion_url: source?.url || undefined,
...
export async function runLegalSearch(
  ...
  const pageResults: PageResult[] = priorPages.flatMap(page => page.records.map(record => ({ page: page.page, position: record.position, case: record.case })));
  ...
  pageLoop: for (let page = request.startPage; !stopReason && page <= request.endPage; page += 1) {
    ...
    try {
      ...
      const normalized = response.results.map((raw, index) => normalizeProviderResult(
        request.provider,
        raw,
        (page - 1) * PROVIDER_PAGE_SIZE + index + 1,
      ));
      if (!cached || needsCapture) recordSearchPage(run, page, response.results, normalized, response.reachedEnd);
      for (let i = pageResults.length - 1; i >= 0; i--) if (pageResults[i].page === page) pageResults.splice(i, 1);
      pageResults.push(...normalized.map((item, index) => ({ page, position: index + 1, case: item })));
      ...
      if (response.reachedEnd === true && !fullScholarCapPage) {
  ...
  const providerPageCapReached = request.provider === "scholar"
    && !providerExhausted
    && resumePage === undefined
    && completedPages.at(-1) === MAX_SEARCH_PAGE;
  ...
  for (const result of outcome.results) {
    result.retrieved_at = latestSearchPages(run).find(page => page.page === result.page)?.retrievedAt;
...
export function legalSearchOutcomeText(outcome: LegalSearchOutcome): string {
  ...
  const lines = [
    ...
    ...outcome.results.map((item, index) => {
      ...
      return `${index + 1}. ${item.title}${metadata ? ` | ${metadata}` : ""} | case_key: ${item.case_key} | page ${item.page}, result ${item.position} | ${item.download_status}${saved}${error}${source}${snippet}`;
    }),
  ];
