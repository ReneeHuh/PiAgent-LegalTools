...
import { ensureDirectory, nowIso, requestFingerprint, slugify, writeJsonAtomic, type NormalizedCase, type ProviderRawResult } from "./core.ts";
...
export function recordSearchPage(run: SearchRun, page: number, raw: ProviderRawResult[], cases: NormalizedCase[], reachedEnd?: boolean): void {
  const retrievedAt = nowIso();
  appendEvent(run, { type: "page", page, retrievedAt, reachedEnd, records: cases.map((item, index) => ({ position: index + 1, raw: raw[index], case: item })) });
