...
`summarize_case` automatically requests strict JSON-schema output through Chat Completions when the selected provider is `lmstudio`, and returns readable Markdown. Other providers retain their configured API. Its three independent analyses run sequentially, followed by the audit and final reconstruction. Calls share a source prefix and per-run cache session; cache reuse depends on provider support. The ordinary pipeline uses five model calls and retries an invalid internal response once per affected stage, for at most ten calls. If it reports exhausted model-output recovery, report the failed stage and model; do not interpret a summary-field JSON error as an invalid `source_path`, repeatedly rerun the same call, or silently substitute Case Chat for the requested summary pipeline.

Opinions above 170,000 estimated tokens use overlapping parts followed by an audit and reconstruction: one call per part plus two, with one possible retry per stage. The host attaches part number/count, original block boundaries, adjacent-part flags, and overlap IDs to each partial and carries them into audit and reconstruction. Those later stages see cited blocks and adjacent context, so they cannot prove the complete opinion contains no omitted issue. Preserve that coverage limitation when describing the result.
