...
Use Justia as supplemental site-search discovery with `jurisdiction: "all"` and no year bounds. The adapter keeps only rendered `law.justia.com/cases/` results and preserves Justia's title, snippet, case path, breadcrumb, page, position, and retrieval time in `provider_data`. Result counts and ordering can change between retrievals, and completing the rendered pages does not establish comprehensive coverage. Justia does not supply cited-by collection through this extension.
