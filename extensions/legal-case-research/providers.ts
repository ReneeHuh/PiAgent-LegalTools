...
import { clickJustiaResult, openJustiaBrowser, searchJustiaPage } from "./provider-justia.ts";
...
export interface JustiaSearchParameters {
  provider: "justia";
  query: string;
  page: number;
}

export type ProviderSearchParameters = ScholarSearchParameters | CourtListenerSearchParameters | JustiaSearchParameters;
...
export function providerSearchParameters(provider: "justia", input: ProviderSearchInput): JustiaSearchParameters;
...
export function providerSearchParameters(provider: DiscoveryProviderId, input: ProviderSearchInput): ProviderSearchParameters {
  ...
  if (provider === "justia") {
    if (!input.query?.trim()) throw new Error("Justia search requires a query.");
    if (input.cites) throw new Error("Justia does not expose cited-by searches.");
    if (input.courts && (typeof input.courts !== "string" || input.courts.trim())) {
      throw new Error('Justia does not expose a reliable court filter; use jurisdiction "all".');
    }
    if (input.year_from !== undefined || input.year_to !== undefined || input.filed_after || input.filed_before) {
      throw new Error("Justia does not expose reliable filing-date filters.");
    }
    return { provider, query: input.query.trim(), page };
  }
...
export async function searchProvider(
  ...
  if (params.provider === "justia") {
    const response = await searchJustiaPage({ query: params.query, page: params.page, navigationSession }, signal, onUpdate);
    return { results: response.results, reachedEnd: response.reachedEnd, lastPage: response.lastPage,
      resultCountText: response.resultCountText, timingMode: response.timingMode };
  }
...
export async function clickProviderResultLink(
  ...
  if (!providerId || (provider !== "justia" && !/^\d+$/.test(providerId))) {
    throw new Error(`${provider} result link must contain a valid opinion identifier.`);
  }
  ...
  const capture = provider === "scholar"
    ...
    : provider === "courtlistener"
      ...
      : await clickJustiaResult(url, directory, signal, onUpdate);
  return {
    provider,
    providerId,
    ...
    providerData: capture.providerData,
  };
}
